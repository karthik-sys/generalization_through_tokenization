...
ARM_META = {  # display order-ish + colors, single source
    ...
    "routed11": ("Copy-Gate", "Bet 1 - pointer mechanism on nlp head, continued from routed8 (regressed)", "var(--bad)"),
    "routed12": ("Deep-Experts", "Bet 2 - per-domain LoRA on top 2 layers, continued from routed8", "var(--routed)"),
    "routed13": ("Precision-Head", "Bet 3 exploratory - margin-trained pathway, continued from routed8", "var(--routed)"),
    "routed14-large": ("Copy-Gate-190M", "Bet 1 at scale, warm-started from routed7 (regressed)", "var(--bad)"),
    "routed15": ("Control-100k", "Round 2 control - no new module, flat LR, continued from routed8 (confirms round 1's gains were training, not mechanism)", "var(--routed)"),
    "routed16": ("Copy-Gate-v2", "Round 2 - genuinely frozen backbone, negative-bias gate (eval harness never exercises the copy path - inconclusive, see note)", "var(--text2)"),
    "routed17": ("Diet-Phase2", "Round 2 - nlp upweighted ~70%, no reinit (best LAMBADA result in the project)", "var(--good)"),
    "routed18": ("Copy-Mining", "Round 2 - diet phase 2 + copy-structure document filter (no gain beyond routed17 alone)", "var(--routed)"),
